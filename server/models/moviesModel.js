...
module.exports = {
    getAllMovies : (callback) => {
        db.query("Select * from movie m inner join moviedetail md on m.movieid = md.movieid inner join movietype mt on md.movietypeid = mt.movietypeid where month(m.Movie_OpenTime) = month(current_date()) and (year(m.Movie_OpenTime) - year(current_date()) = 0)", (err, result) => {
            if(err){
                throw err
            }
            else{
